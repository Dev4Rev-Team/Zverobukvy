...
interface MainMenuViewModel {
    ...
    /**
     * Метод вызывается при изменении имени пользователи (в режиме набора с клавиатуры)
     * в редактируемом item игрока. Данный метод обеспечивает наличие во viewModel актуального
     * имени игрока из редактируемого item игрока.
     */
    ...
    /**
    Метод вызывается при нажатии пользователем кнопки Back (за исключением, когда отображается информационное
    сообщение, тогда для упрощения логики view самостоятельно его закрывает). ViewModel определяет,
    необходимо ли закрывать приложение или необходимо перевести редактируемый элемент игрока  в состояние
    не редактируемого и после этого формирует соответствующее состояние для view.
     */
    fun onBackPressed()
